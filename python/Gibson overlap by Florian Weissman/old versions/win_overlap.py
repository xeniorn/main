...
"""command1 = subprocess.Popen(["E:\PhD\Tools Alati\Gibson overlap by Florian Weissman\RNAfold.exe", "--noGU", "--noPS", "-T 10"], stdin=TempInput, stdout=TempOutput)"""
...
for i in range(0,outputs1):
 ...
 for j in range(0,30):
  if (Tm_dan(seq2[bigger1:seq2.find(seq1[2])+16+j],50) >= primer_tm) and (seq2[seq2.find(seq1[2])+16+j-1].upper() == 'C' or seq2[seq2.find(seq1[2])+16+j-1].upper() =='G'):
   print '>Overlap ' + str(i+1) + ': Template2_for  (Tm ' + str(Tm_dan(seq2[bigger1:seq2.find(seq1[2])+16+j],50))[:4] + ')'
   print seq2[seq2.find(l3[i][0]):seq2.find(seq1[2])+16+j]
   break
 ...
 for j in range(0,30):
  if (Tm_dan(seq2r[bigger1:seq2r.find(seq10r)+16+j],50) >= primer_tm) and (seq2r[seq2r.find(seq10r)+16+j-1].upper() == 'C' or seq2r[seq2r.find(seq10r)+16+j-1].upper() =='G'):
   print '>Overlap ' + str(i+1) + ': Template1_rev  (Tm ' + str(Tm_dan(seq2r[bigger1:seq2r.find(seq10r)+16+j],50))[:4] + ')'
   print seq2r[seq2r.find(l3ir):seq2r.find(seq10r)+16+j]
   break
 print
